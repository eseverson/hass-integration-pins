...
The domain field links straight into the upstream history for that integration: **latest**, **changed since <release>** and **at <release>**. The middle one is usually the one you want — GitHub has no path-filtered compare (comparing two refs across all of core is large enough that it refuses to render the diff), but its commits page does filter by path *and* date, so the release plus the date it shipped gives exactly the commits that touched this integration since then. The date costs nothing: PyPI's release listing, already fetched for the version dropdown, records when each release was uploaded, within minutes of its git tag. The comparison result links each changed file the same way.
